...
function referenceSourceContextText(sourceContext: Record<string, any> | null | undefined): string {
  ...
  const knownPortal =
    sourceContext.known_property_portal === true || sourceContext.known_property_portal === '1';
  if (knownPortal) {
    add('Bekannte Immobilienplattform erkannt', 'ja');
  }
  const directListing =
    sourceContext.direct_listing_url === true || sourceContext.direct_listing_url === '1';
  if (directListing) {
    add('Direkter Inseratslink erkannt', 'ja');
  }
